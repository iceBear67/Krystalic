...
public class Log {
    ...
    public static void info(String mesg) {
        CraftPP.getInst().getLogger().info(FORMAT.replaceAll("%", mesg));
    }

    public static void warn(String mesg) {
        CraftPP.getInst().getLogger().info(FORMAT.replaceAll("%", ChatColor.RED + mesg));
    }

    public static void debug(String mesg) {
        if (!CraftPP.getConf().debug) return;
        CraftPP.getInst().getLogger().info(FORMAT.replaceAll("%", "[D] " + ChatColor.UNDERLINE + mesg));
    }
}
